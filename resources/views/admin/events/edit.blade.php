                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('segment') ? 'has-error' : '' }}">
                            <label class="required" for="segment_id">{{ trans('cruds.event.fields.segment') }}</label>
                            <select class="form-control select2" name="segment_id" id="segment_id" required>
                                @foreach($segments as $id => $segment)
                                    <option value="{{ $id }}" {{ (old('segment_id') ? old('segment_id') : $event->segment->id ?? '') == $id ? 'selected' : '' }}>{{ $segment }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('segment'))
                                <span class="help-block" role="alert">{{ $errors->first('segment') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.event.fields.segment_helper') }}</span>
                        </div>
                        ...
                        <div class="form-group {{ $errors->has('link') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="link" id="link" value="{{ old('link', $event->link) }}">
                            @if($errors->has('link'))
                        ...
                        <div class="form-group {{ $errors->has('start_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="start_date" id="start_date" value="{{ old('start_date', $event->start_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('end_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="end_date" id="end_date" value="{{ old('end_date',$event->end_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('subscripition_period') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="subscripition_period" id="subscripition_period" value="{{ old('subscripition_period', $event->subscripition_period) }}">
                            @if($errors->has('subscripition_period'))
                                <span class="help-block" role="alert">{{ $errors->first('subscripition_period') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.event.fields.subscripition_period_helper') }}</span>
                        </div>
                        ...
                        <div class="form-group {{ $errors->has('enabled') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="enabled" id="enabled" value="1" {{ $event->enabled || old('enabled', 0) === 1 ? 'checked' : '' }}>
                                <label for="enabled" style="font-weight: 400">{{ trans('cruds.event.fields.enabled') }}</label>
                            </div>
                        ...
                        <div class="form-group {{ $errors->has('details') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="details" id="details">{!! old('details', $event->details) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('creator') ? 'has-error' : '' }}">
                            <label for="creator_id">{{ trans('cruds.event.fields.creator') }}</label>
                            <select class="form-control select2" name="creator_id" id="creator_id">
                                @foreach($creators as $id => $creator)
                                    <option value="{{ $id }}" {{ (old('creator_id') ? old('creator_id') : $event->creator->id ?? '') == $id ? 'selected' : '' }}>{{ $creator }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('creator'))
                                <span class="help-block" role="alert">{{ $errors->first('creator') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.event.fields.creator_helper') }}</span>
                        </div>
