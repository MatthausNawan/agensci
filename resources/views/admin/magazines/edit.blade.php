        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.magazine.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.magazines.update", [$magazine->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.magazine.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $magazine->name) }}" required>
                            @if($errors->has('name'))
                                <span class="help-block" role="alert">{{ $errors->first('name') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.magazine.fields.name_helper') }}</span>
                        </div>
                        ...
                        <div class="form-group {{ $errors->has('logo') ? 'has-error' : '' }}">
                            <label for="logo">{{ trans('cruds.magazine.fields.logo') }}</label>
                            <div class="needsclick dropzone" id="logo-dropzone">
                            </div>
                            @if($errors->has('logo'))
                                <span class="help-block" role="alert">{{ $errors->first('logo') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.magazine.fields.logo_helper') }}</span>
                        </div>
...
<script>
    ...
    url: '{{ route('admin.magazines.storeMedia') }}',
    maxFilesize: 2, // MB
    ...
    init: function () {
@if(isset($magazine) && $magazine->logo)
      var file = {!! json_encode($magazine->logo) !!}
