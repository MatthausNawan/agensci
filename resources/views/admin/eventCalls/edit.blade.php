        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.eventCall.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.event-calls.update", [$eventCall->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                            <label for="title">{{ trans('cruds.eventCall.fields.title') }}</label>
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $eventCall->title) }}">
                            @if($errors->has('title'))
                        ...
                        <div class="form-group {{ $errors->has('media') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="media" id="media">{{ old('media', $eventCall->media) }}</textarea>
                            @if($errors->has('media'))
                        ...
                        <div class="form-group {{ $errors->has('media_type') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="media_type" id="media_type">
                                ...
                                @foreach(App\Models\EventCall::MEDIA_TYPE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('media_type', $eventCall->media_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('event') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="event_id" id="event_id">
                                @foreach($events as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('event_id') ? old('event_id') : $eventCall->event->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('event'))
                        ...
                        <div class="form-group {{ $errors->has('creator') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="creator" id="creator" value="{{ old('creator', $eventCall->creator) }}" step="1">
