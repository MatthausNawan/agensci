                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.speakers.update", [$speaker->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $speaker->name) }}">
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('creator') ? 'has-error' : '' }}">
                            <label for="creator_id">{{ trans('cruds.speaker.fields.creator') }}</label>
                            <select class="form-control select2" name="creator_id" id="creator_id">
                                @foreach($creators as $id => $creator)
                                    <option value="{{ $id }}" {{ (old('creator_id') ? old('creator_id') : $speaker->creator->id ?? '') == $id ? 'selected' : '' }}>{{ $creator }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('creator'))
                                <span class="help-block" role="alert">{{ $errors->first('creator') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.speaker.fields.creator_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('bio') ? 'has-error' : '' }}">
                            <label for="bio">{{ trans('cruds.speaker.fields.bio') }}</label>
                            <textarea class="form-control ckeditor" name="bio" id="bio">{!! old('bio', $speaker->bio) !!}</textarea>
                            @if($errors->has('bio'))
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $speaker->description) }}">
                            @if($errors->has('description'))
                                <span class="help-block" role="alert">{{ $errors->first('description') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.speaker.fields.description_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('areas') ? 'has-error' : '' }}">
                            <label for="areas">{{ trans('cruds.speaker.fields.areas') }}</label>
                            <textarea class="form-control" name="areas" id="areas">{{ old('areas', $speaker->areas) }}</textarea>
                            @if($errors->has('areas'))
                                <span class="help-block" role="alert">{{ $errors->first('areas') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.speaker.fields.areas_helper') }}</span>
                        </div>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '/admin/speakers/ckmedia', true);
