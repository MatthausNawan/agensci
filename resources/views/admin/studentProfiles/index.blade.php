@extends('layouts.admin')
@section('content')
<div class="content">
    @can('student_profile_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.student-profiles.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.studentProfile.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
    <div class="row">
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('cruds.studentProfile.title_singular') }} {{ trans('global.list') }}
                </div>
                <div class="panel-body">
                    <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-StudentProfile">
                        <thead>
                            <tr>
                                <th width="10">

                                </th>
                                <th>
                                    {{ trans('cruds.studentProfile.fields.id') }}
                                </th>
                                <th>
                                    {{ trans('cruds.studentProfile.fields.name') }}
                                </th>
                                <th>
                                    {{ trans('cruds.studentProfile.fields.couse_name') }}
                                </th>
                                <th>
                                    {{ trans('cruds.studentProfile.fields.period') }}
                                </th>
                                <th>
                                    {{ trans('cruds.studentProfile.fields.university_name') }}
                                </th>
                                <th>
                                    {{ trans('cruds.studentProfile.fields.lattes_link') }}
                                </th>
                                <th>
                                    {{ trans('cruds.studentProfile.fields.photo') }}
                                </th>
                                <th>
                                    {{ trans('cruds.studentProfile.fields.phone') }}
                                </th>
                                <th>
                                    &nbsp;
                                </th>
                            </tr>
                        </thead>
                    </table>
                </div>
            </div>



        </div>
    </div>
</div>
@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('student_profile_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.student-profiles.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).data(), function (entry) {
          return entry.id
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  let dtOverrideGlobals = {
    buttons: dtButtons,
    processing: true,
    serverSide: true,
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.student-profiles.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'name', name: 'name' },
{ data: 'couse_name', name: 'couse_name' },
{ data: 'period', name: 'period' },
{ data: 'university_name', name: 'university_name' },
{ data: 'lattes_link', name: 'lattes_link' },
{ data: 'photo', name: 'photo', sortable: false, searchable: false },
{ data: 'phone', name: 'phone' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
    ],
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  };
  let table = $('.datatable-StudentProfile').DataTable(dtOverrideGlobals);
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
});

</script>
@endsection